...
EXCLUDE_DIRS = {'node_modules', 'build', 'dist', '.git', 'target', 'out'}
...
"""
    if spdx:
        block = f"// SPDX-License-Identifier: Apache-2.0\n" + block
    return block
...
def has_existing_header(content, ext):
    first_lines = "\n".join(content.splitlines()[:10])
    # Kiểm tra comment header của các loại file
    if ext in ['.md', '.mdx', '.xml', '.html'] and '//' in first_lines:
        return True
    if ext in ['.py', '.yaml', '.yml'] and '#' in first_lines:
        return True
    if ext in ['.java', '.kt', '.ts', '.tsx', '.js', '.jsx', '.css', '.scss'] and '/*' in first_lines:
        return True
    return False
...
# File processing
# =============================
def process_file(path, header, ext):
    try:
    ...
    if has_existing_header(content, ext):
        print(f"Skipping (header exists?): {path}")
        return
...
def walk_and_apply(root, year, owner, spdx):
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for filename in filenames:
            ...
            process_file(full_path, header, ext)
...
def main():
    ...
    parser.add_argument('--spdx', action='store_true', help='Insert SPDX identifier')
